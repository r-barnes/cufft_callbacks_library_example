...
int main(){
    ...
    std::vector<cfloat> vec = {{0.6010,0.0278},{0.4165,0.5814},{0.3336,0.0363},{0.3637,0.9643},{0.8578,0.0192},{0.3967,0.8055},{0.9033,0.5131},{0.1266,0.6511},
    ...
    const std::vector<cfloat> ones = {{1.0000,0.0000},{1.0000,0.0000},{1.0000,0.0000},{1.0000,0.0000},{1.0000,0.0000},{1.0000,0.0000},{1.0000,0.0000},{1.0000,0.0000},
    ...
    cfloat *dvec;
    CheckCudaErrors(cudaMalloc((void **)&dvec, N*N*sizeof(cfloat)));
    CheckCudaErrors(cudaMemcpy(dvec, vec.data(), N*N*sizeof(cfloat), cudaMemcpyHostToDevice));

    cfloat *dones;
    CheckCudaErrors(cudaMalloc((void **)&dones, N*N*sizeof(cfloat)));
    CheckCudaErrors(cudaMemcpy(dones, ones.data(), N*N*sizeof(cfloat), cudaMemcpyHostToDevice));

    scaled_ifft2_inplace(dvec, dones, N);

    CheckCudaErrors(cudaMemcpy(vec.data(), dvec, N*N*sizeof(cfloat), cudaMemcpyDeviceToHost));

    for(int i=0;i<N;i++){
        for(int j=0;j<N;j++){
            std::cout<<vec[j*N+i].x<<","<<vec[j*N+i].y<<"   ";
        }
        std::cout<<"\n";
    }
